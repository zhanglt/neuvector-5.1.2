...
var cis_items = map[string]api.RESTBenchCheck{
	...
	"D.1.1.5": api.RESTBenchCheck{
		...
		Remediation: "为/var/lib/docker目录添加一个规则。",
	},
	...
	"D.3.17": api.RESTBenchCheck{
		...
		Remediation: "如果存在daemon.json文件，则应执行以下命令：chown root:root/etc/docker/daemon.json这将文件的所有权和组所有权设置为root。",
	},
	"D.3.18": api.RESTBenchCheck{
		...
		Remediation: "您应该验证，如果daemon.json存在，其文件权限是否正确设置为644或更严格",
	},
	"D.3.19": api.RESTBenchCheck{
		...
		Remediation: "如果存在该文件，则应执行以下命令：chmod 644/etc/docker/daemon.json这将此文件的文件权限设置为644。",
	},
	"D.3.20": api.RESTBenchCheck{
		...
		Remediation: "您应该执行以下命令：chmod 644/etc/default/docker这将此文件的文件权限设置为644。",
	},
	"D.3.21": api.RESTBenchCheck{
		...
		Remediation: "您应该执行以下命令：chmod 644/etc/sysconfig/docker这将此文件的文件权限设置为644。",
	},
	...
	"D.6.1": api.RESTBenchCheck{
		...
		Profile:     "Level 1",
		Automated:   false,
		Description: "您不应该在同一主机上保留大量的容器映像。仅在适当情况下使用已标记的镜像。",
		Remediation: "您应该只保留您实际需要的图像，并建立一个工作流程以从主机中删除旧的或陈旧的图像。 此外，您应该使用 pull-by-digest 等功能从注册表中获取特定镜像。您可以按照以下步骤在系统中查找未使用的镜像，以便将其删除。",
	},
	"D.6.2": api.RESTBenchCheck{
		...
		Remediation: "您应该定期检查每台主机上的容器清单，并使用以下命令清理未使用的容器：docker container prune",
	},
	...
	"D.5.13": api.RESTBenchCheck{
		...
		Profile:     "Level 1",
		Automated:   false,
		Description: "确保传入的容器流量绑定到特定的主机接口e",
		Remediation: "",
	},
	...
	"D.5.22": api.RESTBenchCheck{
		...
		Profile:     "Level 2",
		Automated:   false,
		Description: "确保 docker exec 命令不与特权选项一起使用n",
		Remediation: "",
	},
	...
	"D.5.29": api.RESTBenchCheck{
		...
		Profile:     "Level 2",
		Automated:   false,
		Description: "确保不使用 Docker 的默认网桥docker0",
		Remediation: "您应该遵循 Docker 文档并设置用户定义的网络。 所有容器都应在此网络中运行。",
	},
	"D.5.30": api.RESTBenchCheck{
		...
		Profile:     "Level 1",
		Automated:   false,
		Description: "确保不共享主机（host）的用户命名空间",
		Remediation: "您不应在主机和容器之间共享用户命名空间。例如，您不应运行以下命令：docker run --rm -it --userns=host ubuntu bash",
	},
	"D.5.31": api.RESTBenchCheck{
		...
		Profile:     "Level 1",
		Automated:   false,
		Description: "确保Docker套接字docker.sock未mount在任何容器内",
		Remediation: "您应该确保没有容器将 docker.sock 作为卷挂载",
	},
	...
	"K.1.2.8": api.RESTBenchCheck{
		...
		Remediation: " 在主节点上编辑 API 服务器 pod 规范文件 /etc/kubernetes/manifests/kube-apiserver.yaml 并将 --authorization-mode 参数设置为包含 Node 的值。 --authorization-mode=节点，RBAC ,--authorization-mode=Node,RBAC",
	},
	...
	"K.1.3.2": api.RESTBenchCheck{
		...
		Automated:   false,
		Description: " 确保 --profiling 参数设置为 false ",
		Remediation: " 在主节点上编辑 Controller Manager pod 规范文件 /etc/kubernetes/manifests/kube-controller-manager.yaml 并设置以下参数。 --分析=假 ",
	},
	...
	"K.3.1.2": api.RESTBenchCheck{
		...
		Profile:     "Level 1",
		Automated:   false,
		Description: " Kubernetes提供的服务帐户令牌旨在供工作负载使用。这些令牌不是为最终用户使用而设计的，也不提供吊销或过期等功能，这使它们不安全。该功能的较新版本（绑定服务帐户令牌卷）确实会导致过期，但仍然不允许特定的吊销。",
		Remediation: " Kubernetes提供的替代机制，如OIDC的使用，应该代替服务帐户令牌来实现。",
	},
	"K.3.1.3": api.RESTBenchCheck{
		TestNum:     "K.3.1.3",
		Type:        "master",
		Category:    "kubernetes",
		Scored:      false,
		Profile:     "Level 1",
		Automated:   false,
		Description: " Kubernetes提供引导令牌，供加入集群的新节点使用。这些令牌不是为最终用户设计的，它们是专门为引导新节点而设计的，而不是用于一般身份验证",
		Remediation: " Kubernetes提供的替代机制，如OIDC的使用，应该代替引导令牌来实现。",
	},
	...
	"K.5.1.1": api.RESTBenchCheck{
		...
		Profile:     "Level 1",
		Automated:   false,
		Description: " RBAC角色集群管理员提供了对环境的广泛权力，应该仅在需要的地方和时间使用。 ",
		Remediation: " 识别到集群管理角色的所有集群角色绑定。检查是否使用了它们，是否需要此角色，或者是否可以使用特权较少的角色。在可能的情况下，首先将用户绑定到特权较低的角色，然后删除与群集管理员角色的clusterrolebinding：kubectl delete clusterrolebinding [name] ",
	},
	"K.5.1.2": api.RESTBenchCheck{
		TestNum:     "K.5.1.2",
		Type:        "master",
		Category:    "kubernetes",
		Scored:      false,
		Profile:     "Level 1",
		Automated:   false,
		Description: " 确保 Kubelet 仅使用强加密密码Kubernetes API存储机密，这些机密可能是Kubernete API的服务帐户令牌，也可能是集群中工作负载使用的凭据。对这些机密的访问应限制在尽可能小的用户组内，以降低权限升级的风险。 ",
		Remediation: " 在可能的情况下，删除对集群中秘密对象的获取、列出和监视访问。 ",
	},
	"K.5.1.3": api.RESTBenchCheck{
		TestNum:     "K.5.1.3",
		Type:        "worker",
		Category:    "kubernetes",
		Scored:      false,
		Profile:     "Level 1",
		Automated:   false,
		Description: " Kubernetes Roles和ClusterRoles基于对象集和可以对这些对象执行的操作来提供对资源的访问。可以将其中任何一个设置为匹配所有项目的通配符“*”。 ",
		Remediation: " 在可能的情况下，将集群角色和角色中通配符的任何使用替换为特定的对象或操作。",
	},
	//-------K5.x没有补充完整
}
